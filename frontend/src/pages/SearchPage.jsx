...
const SearchPage = () => {
    ...
    const handleSearch= async(e)=>
    {
        e.preventDefault()
        try {
            const response = await axios.get(`http://localhost:5000/api/v1/search/${activeTab}/${searchTerm}`, {
    ...
    <div className='bg-black min-h-screen text-white' >
      ...
      <div className='container mx-auto px-4 py-8'>
        ...
        <form className='flex gap-2 items-stretch mb-8 max-w-2xl mx-auto' onSubmit={handleSearch}>
            <input type='text' className='p-2 bg-gray-800  text-white rounded w-full' placeholder={'Search for ' + activeTab} value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)}/>
            <button className='py-2 px-4 rounded bg-red-600 hover:bg-red-700 text-white' >
